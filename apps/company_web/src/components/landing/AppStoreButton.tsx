...
/**
 * Reusable store download button used across the landing page.
 */
export default function AppStoreButton({
  ...
  const palette = tone === 'light' ? 'bg-white text-green-700 hover:bg-white/90 shadow' : 'bg-black text-white hover:bg-gray-800 shadow-lg'
  ...
  if (href) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className={clsx(baseClasses, palette, className)}>
        {content}
      </a>
    )
  }
  ...
  return (
    <button type="button" onClick={onClick} className={clsx(baseClasses, palette, className)}>
      {content}
    </button>
  )
}
